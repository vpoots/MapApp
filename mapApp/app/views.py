...
def restData():
	try:
		qpylib.log("hi")
		headers = {'content-type' : 'text/plain'}
		searchQuery = {'query_expression' : "select * from \"flows\"",}
...
def flowData(statusSearch_id):
	try:
		...
		#sourceIpAddData = json.dumps(geoIpData, indent = 2)
		#response = HttpResponse(json.dumps(geoIpData), content_type="application/json")

		#return JsonResponse(geoIpData), 'HTML':
		return render_template("map.html", data=json.dumps(geoIpData))
